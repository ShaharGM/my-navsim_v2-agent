...
class WeightedEgoRecogDriveFeatureBuilder(AbstractFeatureBuilder):
    def __init__(self,
    ...
    def compute_features(self, agent_input: AgentInput) -> Dict[str, torch.Tensor]:
        ...
        # NOTE - we assume 4 historical ego statuses and 8 future ego statuses here
        ...
        high_command_one_hot = torch.tensor(ego_statuses[3].driving_command, dtype=torch.float32)
        # Compute future ego statuses
        future_list = []
        for e in ego_statuses[4:]:
            future_tensor = torch.cat([
                ...
                torch.tensor(e.ego_pose, dtype=torch.float32)
            ], dim=-1)
            future_list.append(future_tensor)
        future_ego_statuses = torch.stack(future_list) if future_list else torch.empty(0, 11)

        if not self.cache_hidden_state:
            image_path = str(cameras[-1].cam_f0.camera_path)
            ...
            return {
                ...
                "future_ego_statuses": future_ego_statuses.cpu(),
            ...
            pixel_values = load_image(Image.fromarray(cameras[-1].cam_f0.image.astype(np.uint8))).unsqueeze(0)
            ...
            # TODO had to adjust their code to make it run with decent memory usage
