...
class Diffusion(nn.Module):
    ...
    def model_predictions(self, x, t, external_cond=None):
        x_in = x.transpose(0, 1)  # (seq_len, batch, dim)
        t_in = t.transpose(0, 1)  # (seq_len, batch)
        external_cond_in = external_cond.transpose(0, 1) if external_cond is not None else None

        model_output = self.model(x_in, t_in, external_cond_in, is_causal=self.is_causal)

        # Transpose back to batch-first
        model_output = model_output.transpose(0, 1)  # (batch, seq_len, dim)
    ...
    def compute_loss_weights(self, noise_levels: torch.Tensor):
        ...
        seq_len = normalized_snr.shape[1]
        for s in range(seq_len):
            if s == 0:
                cum_snr[:, s] = normalized_clipped_snr[:, s]
            else:
                cum_snr[:, s] = self.cum_snr_decay * cum_snr[:, s - 1] + (1 - self.cum_snr_decay) * normalized_clipped_snr[:, s]
