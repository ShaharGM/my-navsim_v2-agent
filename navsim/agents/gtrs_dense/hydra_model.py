...
class HydraModel(nn.Module):
    ...
    def forward(self, features: Dict[str, torch.Tensor],
                interpolated_traj=None, topk=1) -> Dict[str, torch.Tensor]:
        status_feature: torch.Tensor = features["status_feature"]
        camera_feature = features["camera_feature"]
        ...
        output: Dict[str, torch.Tensor] = {}
        trajectory = self._trajectory_head(keyval, status_encoding, interpolated_traj, topk=topk)
        output.update(trajectory)
        return output
...
class HydraTrajHead(nn.Module):
    ...
    def forward(self, bev_feature, status_encoding, interpolated_traj=None, topk=1) -> Dict[str, torch.Tensor]:
        result = {}
        # vocab: 4096, 40, 3
        # bev_feature: B, 32, C
        ...
        # NOTE my edit to allow for top-k selection
        k_val = max(topk, 1)
        # print(f"Takes top-k with k={k_val}")
        topk_vals, topk_inds = torch.topk(scores, k=k_val, dim=1)

        best_indices = topk_inds[:, 0]
        
        # Fix bug where if we do a dropout on our vocab, we take the wrong trajectories since the indices selected are for the subset of the vocab
        current_vocab = result.get('trajectory_vocab_dropout', self.vocab.data)
        
        result["selected_indices"] = best_indices          # Shape: [B]
        result["trajectory"] = current_vocab[best_indices] # Shape: [B, 40, 3]
        result["trajectory_vocab"] = self.vocab.data
        
        # We only add these if relevant to save dict clutter, or always add them if you prefer
        result["topk_indices"] = topk_inds                 # Shape: [B, K]
        result["topk_scores"] = topk_vals                  # Shape: [B, K]
    ...
    def eval_dp_proposals(self, bev_feature,
        ...
        # # Uncomment to only use proposals for the scorer's context
        # L = 0
        # vocab = dp_proposals.view(B, NUM_PROPOSALS, -1)
        ...
        if dp_only_inference:
            topk_vals, topk_inds = torch.topk(scores[:, L:], k=k_val, dim=1)
            topk_inds = topk_inds + L
        else:
            topk_vals, topk_inds = torch.topk(scores, k=k_val, dim=1)
        ...
        result["topk_indices"] = topk_inds                 # Shape: [B, K]
        result["topk_scores"] = topk_vals                  # Shape: [B, K]
        result["selected_indices"] = best_indices
        result["trajectory"] = vocab[scene_cnt_tensor, best_indices].view(B, HORIZON, 3)
