...
class GTRSAgent(AbstractAgent):
    ...
    def initialize(self) -> None:
        ...
        print('GTRS-Dense model loaded', msg)
    ...
    def forward_topk(self, features: Dict[str, torch.Tensor], topk) -> Dict[str, torch.Tensor]:
        return self.model(features, topk=topk)

    def evaluate_dp_proposals(self, features: Dict[str, torch.Tensor], dp_proposals, dp_only_inference=False, topk=10) -> Dict[str, torch.Tensor]:
        return self.model.evaluate_dp_proposals(features, dp_proposals, dp_only_inference=dp_only_inference, topk=topk)
