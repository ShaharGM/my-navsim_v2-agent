...
from .ego_router import EgoRouter
...
class ReCogDriveAgent(AbstractAgent):
    def __init__(
        ...
        # Since I am not saving hidden states for storage purposes, I am using the feature builder to calculate per input. Slower inference.
        # I am not using their "internal backbone" because it doesnt make sense not to use the same setup as they do for their cached runs.
        # The feature builder uses the same backbone, with better prompts.
        ...
        # TODO fix later to enable cuda support for different ray workers
        ...
        # TODO fully implement the ego router model and adjust inputs (or use config file)
        self.ego_router = EgoRouter(hidden_layer_dim = 128, lr=0.0, ego_status_count=8, trajectory_sampling=trajectory_sampling)
    ...
    def get_feature_builders(self) -> List[AbstractFeatureBuilder]:
        return [ReCogDriveFeatureBuilder(
            ...
            checkpoint_path=self.vlm_path,
            cache_mode=self.cache_mode,
        )] if not self.feature_builder else [self.feature_builder]
    ...
    def compute_trajectory_old(self, features: Dict[str, torch.Tensor]) -> Trajectory:
        self.eval()
        with torch.no_grad():
    ...
    def get_optimizers(self) -> Union[Optimizer, Dict[str, LRScheduler]]:
        ...
        optimizer = build_from_configs(optim, optimizer_cfg, params=self.ego_router.parameters())
