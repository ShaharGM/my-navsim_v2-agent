...
from navsim.agents.diffusion_transfuser.diffusion_transfuser_loss import diffusion_transfuser_loss
from navsim.common.dataclasses import SensorConfig
from navsim.planning.training.abstract_feature_target_builder import AbstractFeatureBuilder, AbstractTargetBuilder


class DiffusionTransfuserAgent(AbstractAgent):
    """Agent interface for Diffusion TransFuser."""

    def __init__(
        self,
        config: DiffusionTransfuserConfig,
        lr: float,
        checkpoint_path: Optional[str] = None,
        trajectory_sampling: TrajectorySampling = TrajectorySampling(time_horizon=4, interval_length=0.5),
    ):
        """
        Initializes Diffusion TransFuser agent.
        :param config: global config of Diffusion TransFuser agent
        ...
        self._diffusion_transfuser_model = DiffusionTransfuserModel(self._trajectory_sampling, config)
    ...
    def initialize(self) -> None:
        ...
        #     self._diffusion_transfuser_model = self._diffusion_transfuser_model.cuda()
    ...
    def forward(self, features: Dict[str, torch.Tensor], targets=None) -> Dict[str, torch.Tensor]:
        """Inherited, see superclass."""
        return self._diffusion_transfuser_model(features, targets)
    ...
    def compute_loss(
        ...
        """Inherited, see superclass."""
        transfuser_loss = diffusion_transfuser_loss(targets, predictions, self._config, self._diffusion_transfuser_model)
        return transfuser_loss + (self._config.diffusion_weight * predictions["diffusion_loss"])

    def get_optimizers(
        self,
    ) -> Union[Optimizer, Dict[str, Union[Optimizer, LRScheduler]]]:
        """Inherited, see superclass."""
        return torch.optim.Adam(self._diffusion_transfuser_model.parameters(), lr=self._lr)
