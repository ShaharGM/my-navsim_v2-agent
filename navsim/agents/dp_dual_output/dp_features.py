...
class HydraFeatureBuilder(AbstractFeatureBuilder):
    ...
    def compute_features(self, agent_input: AgentInput) -> Dict[str, torch.Tensor]:
        ...
        features["status_feature"] = torch.cat(ego_status_list)
        features['ego_pose'] = torch.tensor(agent_input.ego_statuses[-1].ego_pose, dtype=torch.float32)
        features['ego_velocity'] = torch.tensor(agent_input.ego_statuses[-1].ego_velocity, dtype=torch.float32)
        features['ego_acceleration'] = torch.tensor(agent_input.ego_statuses[-1].ego_acceleration, dtype=torch.float32)
        return features
...
class HydraTargetBuilder(AbstractTargetBuilder):
    ...
    def compute_targets(self, scene: Scene) -> Dict[str, torch.Tensor]:
        ...
        future_ego_statuses = scene.get_future_ego_statuses()

        trajectory = torch.tensor(future_traj.poses)

        matching_statuses = future_ego_statuses[:len(trajectory)]

        # Extract [vx, vy, ax, ay] from each status object
        dynamics_data = [
            [s.ego_velocity[0], s.ego_velocity[1], s.ego_acceleration[0], s.ego_acceleration[1]]
            for s in matching_statuses
        ]
        
        # Convert to tensor matching the trajectory's type/device
        dynamics_tensor = torch.tensor(dynamics_data, dtype=trajectory.dtype, device=trajectory.device)

        # Concatenate along the feature dimension: (8, 3) + (8, 4) -> (8, 7)
        trajectory = torch.cat([trajectory, dynamics_tensor], dim=-1)
