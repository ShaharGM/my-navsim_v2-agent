...
# --- 1. Position Difference Statistics ---
...
# --- 4. Pre-calculate Scaling Factors for Speed ---
# Position Scales
...
# Velocity Scales
vx_scale = max(abs(vx_max - vx_mean), abs(vx_min - vx_mean))
vy_scale = max(abs(vy_max - vy_mean), abs(vy_min - vy_mean))

# Acceleration Scales
ax_scale = max(abs(ax_max - ax_mean), abs(ax_min - ax_mean))
ay_scale = max(abs(ay_max - ay_mean), abs(ay_min - ay_mean))

HORIZON = 8 # 4 seconds 2Hz
ACTION_DIM = 8 # (x, y, sin, cos, ax, ay, vx, vy)
ACTION_DIM_ORI = 7 # (x, y, heading, ax, ay, vx, vy)
...
def diff_traj(traj):
    """
    Normalizes trajectory targets for training.
    Input traj shape: [Batch, Length, 7] -> (x, y, heading, vx, vy, ax, ay)
    Output shape:     [Batch, Length, 8] -> (dx_norm, dy_norm, sin, cos, vx_norm, vy_norm, ax_norm, ay_norm)
    """
    B, L, _ = traj.shape
    
    # --- 1. Position (Convert absolute x,y to relative deltas) ---
    zero_pad = torch.zeros((B, 1, 1), dtype=traj.dtype, device=traj.device)
    
    x_diff = traj[..., 0:1].diff(n=1, dim=1, prepend=zero_pad)
    x_diff_norm = (x_diff - x_diff_mean) / x_diff_scale

    y_diff = traj[..., 1:2].diff(n=1, dim=1, prepend=zero_pad)
    y_diff_norm = (y_diff - y_diff_mean) / y_diff_scale

    # --- 2. Heading (Convert to Sin/Cos) ---
    sin = traj[..., 2:3].sin()
    cos = traj[..., 2:3].cos()

    # --- 3. Velocity (Normalize directly - No diff needed) ---
    vx_norm = (traj[..., 3:4] - vx_mean) / vx_scale
    vy_norm = (traj[..., 4:5] - vy_mean) / vy_scale

    # --- 4. Acceleration (Normalize directly - No diff needed) ---
    ax_norm = (traj[..., 5:6] - ax_mean) / ax_scale
    ay_norm = (traj[..., 6:7] - ay_mean) / ay_scale

    # Concatenate all features
    return torch.cat([x_diff_norm, y_diff_norm, sin, cos, vx_norm, vy_norm, ax_norm, ay_norm], -1)
...
def cumsum_traj(norm_trajs):
    """
    Denormalizes model outputs back to physical values.
    Input shape:  [Batch, Length, 8]
    Output shape: [Batch, Length, 7] -> (x, y, heading, vx, vy, ax, ay)
    """
    B, L, _ = norm_trajs.shape

    # --- 1. Position (Denormalize deltas + Cumulative Sum) ---
    x_diff = norm_trajs[..., 0:1] * x_diff_scale + x_diff_mean
    y_diff = norm_trajs[..., 1:2] * y_diff_scale + y_diff_mean
    
    x = x_diff.cumsum(dim=1)
    y = y_diff.cumsum(dim=1)

    # --- 2. Heading (Atan2 of Sin/Cos) ---
    sin_values = norm_trajs[..., 2:3]
    cos_values = norm_trajs[..., 3:4]
    heading = torch.atan2(sin_values, cos_values)

    # --- 3. Velocity (Denormalize directly) ---
    vx = norm_trajs[..., 4:5] * vx_scale + vx_mean
    vy = norm_trajs[..., 5:6] * vy_scale + vy_mean

    # --- 4. Acceleration (Denormalize directly) ---
    ax = norm_trajs[..., 6:7] * ax_scale + ax_mean
    ay = norm_trajs[..., 7:8] * ay_scale + ay_mean

    # Concatenate to 7D output
    return torch.cat([x, y, heading, vx, vy, ax, ay], -1)
...
class DPModel(nn.Module):
    ...
    def forward(self, features: Dict[str, torch.Tensor],
        ...
        # print(f"camera_feature: {camera_feature.shape}")
        # print(f"camera_feature_back: {camera_feature_back.shape}")
        # print(f"status_feature: {status_feature.shape}")
        ...
        # print(f"camera_feature_curr: {camera_feature_curr.shape}")
        # print(f"camera_feature_back_curr: {camera_feature_back_curr.shape}")
        ...
        head_out = self._trajectory_head(keyval)
        
        selected_traj = torch.zeros(batch_size, HORIZON, ACTION_DIM_ORI)
        
        if not self.training:
            # print(f"dp head output shape {head_out['dp_pred'].shape}")
        ...
        # Also return the who bunch of generated trajectories
