...
class TransfuserFeatureBuilder(AbstractFeatureBuilder):
    ...
    def _get_lidar_feature(self, agent_input: AgentInput) -> torch.Tensor:
        ...
        """
        if not agent_input.lidars or agent_input.lidars[-1] is None or agent_input.lidars[-1].lidar_pc is None:
            # No LiDAR data, return zero tensor of correct shape
            print("No lidar data...")
            xbins = np.linspace(
            ...
            H = len(ybins) - 1
            W = len(xbins) - 1
            channels = 2 if self._config.use_ground_plane else 1
            return torch.zeros(channels, H, W, dtype=torch.float32)
