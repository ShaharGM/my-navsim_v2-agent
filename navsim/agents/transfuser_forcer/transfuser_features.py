...
class TransfuserTargetBuilder(AbstractTargetBuilder):
    ...
    def compute_targets(self, scene: Scene) -> Dict[str, torch.Tensor]:
        ...
        frame_idx = scene.scene_metadata.num_history_frames - 1
        future_frames = scene.frames[frame_idx + 1 : frame_idx + 1 + self._trajectory_sampling.num_poses]
        velocities = torch.stack([torch.tensor(f.ego_status.ego_velocity) for f in future_frames])
        accelerations = torch.stack([torch.tensor(f.ego_status.ego_acceleration) for f in future_frames])

        # Concatenate trajectory, velocities, accelerations
        extended_trajectory = torch.cat([trajectory, velocities, accelerations], dim=-1)
