...
from navsim.agents.transfuser_forcer.transfuser_backbone import TransfuserBackbone
from navsim.agents.transfuser_forcer.transfuser_config import TransfuserConfig
from navsim.agents.transfuser_forcer.transfuser_features import BoundingBox2DIndex
...
from navsim.agents.transfuser_forcer.modules.conditional_unet1d import SinusoidalPosEmb
from navsim.agents.transfuser_forcer.modules.blocks import linear_relu_ln,bias_init_with_prob, gen_sineembed_for_position, GridSampleCrossBEVAttention
from navsim.agents.transfuser_forcer.modules.multimodal_loss import LossComputer


class TransfuserForcerModel(nn.Module):
    ...
    def __init__(self, trajectory_sampling: TrajectorySampling, config: TransfuserConfig):
        ...
        print(f"Transfuser backbone latent = {config.latent}")
    ...
    def forward(self, features: Dict[str, torch.Tensor], targets: Dict[str, torch.Tensor]=None) -> Dict[str, torch.Tensor]:
        ...
        # NOTE DiffDrive addition to use as condition for the diffusion trajectory head
...
class DiffMotionPlanningRefinementModule(nn.Module):
    def __init__(
        ...
        self.plan_reg_branch = nn.Sequential(
            ...
            nn.Linear(embed_dims, ego_fut_ts * 7),  # Changed to 7: x, y, heading, vx, vy, ax, ay
        )
    ...
    def forward(
        ...
        plan_reg = traj_delta.reshape(bs,ego_fut_mode, self.ego_fut_ts, 7)
...
class CustomTransformerDecoderLayer(nn.Module):
    ...
    def forward(self, 
        ...
        poses_reg[..., :2] = poses_reg[..., :2] + noisy_traj_points[..., :2]
...
class TrajectoryHead(nn.Module):
    ...
    def __init__(self, num_poses: int, d_ffn: int, d_model: int, plan_anchor_path: str,config: TransfuserConfig):
        ...
        # Pad plan_anchor to include heading, vx, vy, ax, ay (assuming original is x,y)
        if plan_anchor.shape[-1] == 2:
            # Add heading (0), vx (0), vy (0), ax (0), ay (0)
            padding = np.zeros((plan_anchor.shape[0], plan_anchor.shape[1], 5))
            plan_anchor = np.concatenate([plan_anchor, padding], axis=-1)

        self.plan_anchor = nn.Parameter(
            torch.tensor(plan_anchor, dtype=torch.float32),
            requires_grad=False,
        ) # 20,8,7 (after padding)

        print(f"Plan anchor shape: {self.plan_anchor.shape}")
    ...
    def norm_odo(self, odo_info_fut):
        ...
        odo_info_fut_vx = odo_info_fut[..., 3:4]
        odo_info_fut_vy = odo_info_fut[..., 4:5]
        odo_info_fut_ax = odo_info_fut[..., 5:6]
        odo_info_fut_ay = odo_info_fut[..., 6:7]
        ...
        odo_info_fut_vx = 2*(odo_info_fut_vx + 10)/20 -1  # Assuming vx in [-10, 10]
        odo_info_fut_vy = 2*(odo_info_fut_vy + 10)/20 -1  # Assuming vy in [-10, 10]
        odo_info_fut_ax = 2*(odo_info_fut_ax + 5)/10 -1  # Assuming ax in [-5, 5]
        odo_info_fut_ay = 2*(odo_info_fut_ay + 5)/10 -1  # Assuming ay in [-5, 5]

        return torch.cat([odo_info_fut_x, odo_info_fut_y, odo_info_fut_head, odo_info_fut_vx, odo_info_fut_vy, odo_info_fut_ax, odo_info_fut_ay], dim=-1)
    ...
    def denorm_odo(self, odo_info_fut):
        ...
        odo_info_fut_vx = (odo_info_fut_vx + 1)/2 * 20 - 10
        odo_info_fut_vy = (odo_info_fut_vy + 1)/2 * 20 - 10
        odo_info_fut_ax = (odo_info_fut_ax + 1)/2 * 10 - 5
        odo_info_fut_ay = (odo_info_fut_ay + 1)/2 * 10 - 5
    ...
    def forward_train(self, ego_query,agents_query,bev_feature,bev_spatial_shape,status_encoding, targets=None,global_img=None) -> Dict[str, torch.Tensor]:
        ...
        mode_idx = mode_idx[...,None,None,None].repeat(1,1,self._num_poses,7)
        ...
        return {"trajectory": best_reg[..., :3],"trajectory_loss":ret_traj_loss,"trajectory_loss_dict":trajectory_loss_dict}
    ...
    def forward_test(self, ego_query,agents_query,bev_feature,bev_spatial_shape,status_encoding,global_img) -> Dict[str, torch.Tensor]:
        ...
        # 1. add truncated noise to the plan anchor
        plan_anchor = self.plan_anchor.unsqueeze(0).repeat(bs,1,1,1)
        # print(f"plan anchor: {plan_anchor}")
        norm_anchors = self.norm_odo(plan_anchor)
        noise = torch.randn(norm_anchors.shape, device=device)
        trunc_timesteps = torch.ones((bs,), device=device, dtype=torch.long) * 8
        noised_norm_anchors = self.diffusion_scheduler.add_noise(original_samples=norm_anchors, noise=noise, timesteps=trunc_timesteps)
        noisy_trajs = self.denorm_odo(noised_norm_anchors)
        ego_fut_mode = noised_norm_anchors.shape[1]
        for k in roll_timesteps[:]:
            x_boxes = torch.clamp(noised_norm_anchors, min=-1, max=1)
            ...
            if not torch.is_tensor(timesteps):
                ...
                timesteps = torch.tensor([timesteps], dtype=torch.long, device=noised_norm_anchors.device)
            elif torch.is_tensor(timesteps) and len(timesteps.shape) == 0:
                timesteps = timesteps[None].to(noised_norm_anchors.device)
            
            # 3. embed the timesteps
            timesteps = timesteps.expand(noised_norm_anchors.shape[0])
            time_embed = self.time_mlp(timesteps)
            time_embed = time_embed.view(bs,1,-1)
            ...
            # print(f"Model output at timestep {k}:\nPoses: {poses_reg}\nTraj. CLS: {poses_cls}")
            
            # x_start = poses_reg[...,:2]
            x_start = poses_reg
            x_start = self.norm_odo(x_start)
            noised_norm_anchors = self.diffusion_scheduler.step(
                model_output=x_start,
                timestep=k,
                sample=noised_norm_anchors
            ).prev_sample
