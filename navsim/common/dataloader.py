...
from navsim.common.dataclasses import AgentInput, Lidar, Scene, SceneFilter, SensorConfig
...
class SceneLoader:
    ...
    def __init__(
        ...
        self._final_to_current = dict(zip(stage1_scenes_final_frames_tokens, self.scene_frames_dicts.keys()))
    ...
    def _transform_lidar_for_synthetic_scene(
        self, scene: Scene, orig_frame: Dict, original_lidar_path: Path, current_frame_idx: int
    ) -> None:
        """
        Transform lidar points from original scene to synthetic scene ego frame.
        :param scene: The synthetic scene object.
        :param orig_frame: The original frame dictionary.
        :param original_lidar_path: Path to the original lidar PCD file.
        :param current_frame_idx: Index of the current frame in the scene.
        """
        try:
            with open(original_lidar_path, 'rb') as f:
                buffer = io.BytesIO(f.read())
            orig_points = LidarPointCloud.from_buffer(buffer, "pcd").points
            orig_ego_status = Scene._build_ego_status(orig_frame)
            orig_array = orig_ego_status.ego_pose
            synth_array = scene.frames[current_frame_idx].ego_status.ego_pose
            # Compute relative pose from orig to synth
            theta = -orig_array[2]
            R = np.array([[np.cos(theta), -np.sin(theta)], [np.sin(theta), np.cos(theta)]])
            rel_array = synth_array - orig_array
            rel_array[:2] = rel_array[:2] @ R.T
            rel_array[2] = np.arctan2(np.sin(rel_array[2]), np.cos(rel_array[2]))
            # Apply transformation to points
            points_2d = orig_points[:, :2]
            cos_h = np.cos(rel_array[2])
            sin_h = np.sin(rel_array[2])
            R_trans = np.array([[cos_h, -sin_h], [sin_h, cos_h]])
            transformed_2d = points_2d @ R_trans + rel_array[:2]
            transformed_points = np.column_stack((transformed_2d, orig_points[:, 2:]))
            scene.frames[current_frame_idx].lidar = Lidar(transformed_points, None)
        except (ImportError, ModuleNotFoundError, FileNotFoundError, KeyError, IndexError, ValueError):
            print(f"Could not load or transform lidar for synthetic scene. Lidar will be set to None.")
    ...
    def get_scene_from_token(self, token: str) -> Scene:
        ...
        if token in self.synthetic_scenes:
            ...
            current_frame_idx = self._scene_filter.num_history_frames - 1
            if "lidar_pc" in self._sensor_config.get_sensors_at_iteration(current_frame_idx):
                print(f"Generating lidar for token {token}")
                # Transform lidar for synthetic scenes if missing
                orig_token = scene.scene_metadata.corresponding_original_scene
                log_name = scene.scene_metadata.log_name
                orig_scene_token = self._final_to_current[orig_token]
                orig_frame = self.scene_frames_dicts[orig_scene_token][self._scene_filter.num_frames - 1]
                original_lidar_path = self._original_sensor_path / f"{log_name}" / "MergedPointCloud" / f"{orig_token}.pcd"
                if original_lidar_path.exists():
                    self._transform_lidar_for_synthetic_scene(scene, orig_frame, original_lidar_path, current_frame_idx)
            return scene
        else:
