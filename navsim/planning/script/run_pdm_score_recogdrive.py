...
def run_pdm_score(args: List[Dict[str, Union[List[str], DictConfig]]]) -> List[pd.DataFrame]:
    ...
    for idx, (token) in enumerate(tokens_to_evaluate_stage_one):
        ...
        try:
            ...
            trajectory = agent.compute_trajectory_old(features)
...
def main(cfg: DictConfig) -> None:
    ...
    # works for multi worker gpus, hard coded for x chunks (when x should be exactly the num of threads per node in 'ray_distributed' worker mode)
    num_chunks = cfg.worker.threads_per_node
    object_chunks = [data_points[i::num_chunks] for i in range(num_chunks)]
    score_rows: List[List[pd.DataFrame]] = worker.map(Task(fn=run_pdm_score, num_gpus=1, num_cpus=1), object_chunks)

    pdm_score_df = pd.concat([df for chunk in score_rows for df in chunk])

    # # only works for multi workers without gpus, so for gpu heavy model like recogdrive it will only work in 'sequential' worker mode
