...
from nuplan.common.geometry.convert import relative_to_absolute_poses, change_se2_reference_frame
...
from navsim.common.dataclasses import PDMResults, SensorConfig, Lidar
...
def transform_points(points: np.ndarray, transformation: StateSE2) -> np.ndarray:
    """
    Apply SE(2) transformation to 3D points (x, y, z).
    Assumes points are in world coordinates.
    """
    # Extract 2D points for SE(2) transform
    points_2d = points[:, :2]
    # Apply transformation
    transformed_2d = change_se2_reference_frame(points_2d, transformation)
    # Reconstruct 3D with unchanged z
    return np.column_stack((transformed_2d, points[:, 2]))
...
def run_pdm_score(args: List[Dict[str, Union[List[str], DictConfig]]]) -> List[pd.DataFrame]:
    ...
    for token in scene_loader.tokens:
        if scene_loader.is_synthetic_token(token):
            orig_token, prev_token, two_stage_pairs = scene_loader.get_two_stage_info(token)
            all_mappings[(orig_token, prev_token)] = [tuple(pair) for pair in two_stage_pairs]

    synth_to_orig = {}
    for (orig, prev), pairs in all_mappings.items():
        for now, prev_s in pairs:
            synth_to_orig[now] = orig
            synth_to_orig[prev_s] = prev
    ...
    for idx, (token) in enumerate(tokens_to_evaluate_stage_two):
        ...
        try:
            ...
            if agent.requires_scene:
                scene = scene_loader.get_scene_from_token(token)
                # Add lidar for synthetic scenes if needed
                if token in synth_to_orig and scene.frames[0].lidar is None:
                    orig_token = synth_to_orig[token]
                    orig_pickle_path = Path(cfg.navsim_log_path) / f"{orig_token}.pickle"
                    with open(orig_pickle_path, 'rb') as f:
                        orig_data = pickle.load(f)
                    orig_frame = orig_data['scene_dict_list'][0]
                    orig_lidar_path = orig_frame.get('lidar_path')
                    if orig_lidar_path:
                        full_orig_lidar_path = Path(cfg.original_sensor_path) / orig_lidar_path
                        try:
                            import pypcd
                            pc = pypcd.PointCloud.from_path(str(full_orig_lidar_path))
                            orig_points = pc.pc_data[['x', 'y', 'z']].to_numpy()
                            orig_pose = StateSE2(*orig_frame['ego_pose'])
                            synth_pose = StateSE2(*scene.frames[0].ego_status.ego_pose)
                            transformation = synth_pose - orig_pose
                            transformed_points = transform_points(orig_points, transformation)
                            scene.frames[0].lidar = Lidar(transformed_points, None)
                            agent_input.lidars[0] = scene.frames[0].lidar  # Update agent_input too
                        except (ImportError, FileNotFoundError) as e:
                            logger.warning(f"Failed to load/transform lidar for {token}: {e}")
                trajectory = agent.compute_trajectory(agent_input, scene)
            else:
                trajectory = agent.compute_trajectory(agent_input)
