...
class Dataset(torch.utils.data.Dataset):
    ...
    def _cache_scene_with_token(self, token: str) -> None:
        ...
        # # NOTE added an option to skip missing tokens in the dataset for the loader
        # try:
        #     scene = self._scene_loader.get_scene_from_token(token)
        # except FileNotFoundError as e:
        #     logging.warning(f"Skipping scene {token} due to missing file: {e}")
        #     return
        ...
        # # TODO uncomment inorder to add future ego statuses to the agent input
        # agent_input.ego_statuses = scene.get_all_ego_statuses()
